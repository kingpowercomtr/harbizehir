...
import type { AdminOrder } from "@/components/admin/AdminOrdersTable";
...
export default function AdminEditOrderModal({ order, onClose, onSaved }: AdminEditOrderModalProps) {
  ...
  useEffect(() => {
    if (order) {
      // Telefon "+905..." formatından 10 haneliye çevir
      const digits = order.phone.replace(/\D/g, "");
      const localPhone = digits.length > 10 ? digits.slice(-10) : digits;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <h2 className="text-xl font-bold text-white mb-1">Siparişi Düzenle</h2>
        <p className="text-gray-500 text-sm mb-5 font-mono">{order.code}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            >
              ...
              {!ORDER_STATUSES.some((s) => s.value === form.status) && (
