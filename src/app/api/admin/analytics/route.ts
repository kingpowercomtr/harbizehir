...
export async function GET() {
  ...
  try {
    ...
    const startOfPrevWeek = new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000).toISOString();
    ...
    const [
      ...
      db.execute({ sql: `SELECT "packageType", "packageLabel", COUNT(*) as count, SUM(price) as total FROM "Order" GROUP BY "packageType", "packageLabel" ORDER BY count DESC`, args: [] }),
